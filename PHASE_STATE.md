...
1. **Attack the $\mathsf{TC}^0$-SAT hinge (now precisely located).** A 2026-06-03
   ...
   machineries are now mapped as dead, each with a quantified wall. J1 is the most-attackable joint, and its in-model bulk + co-nd analysis is NOT yet
   complete: exactly one residual object remains, the SUB-BLOCK NON-LINEAR PER-ROUND
   INFORMATION LEMMA, now confirmed OPEN, not partial (finding 33,
   `experiments/circuit_complexity/e_j1_subblock_j3_locality.py`). Route (a)
   (Simchowitz STOC 2018 / Kacham-Woodruff NeurIPS 2023) is linear-only and dead for
   spectral reads; route (b)'s closing telescope was a MEASUREMENT ARTIFACT (an
   inconsistent $\Theta(d^2)$ signal model, not the gap-1-faithful $+(2d-1)$
   increment), so even the marginal half is unestablished. The named missing object is
   per-round CONDITIONAL location-info uniformity $I(\mathrm{ans}_k; L \mid
   \mathrm{ans}_{<k}) = O(b_k^2/n^2)$ under adaptive sub-block choice; the most-
   promising route is BGLWWZ MIC via streaming (arXiv:2403.20283, a SPACE bound
   agnostic to query linearity), needing a streaming-simulation lemma and a gap-1-to-
   needle/MostlyEq reduction (NOT UDISJ). J3, the BINDING joint, is now ATTACKED-AND-
   MAPPED (finding 34): the locality barrier (CHOPRS JACM 2022 Def 11 / Thm 2; GIIKKT
   ICALP 2019) is an OPEN NO-GO covering the polynomial method, approximate degree,
   random restrictions, reductions, and the approximation method (Pich, Comp.
   Complexity 2024), downstream of natural proofs. The evasion criterion is named: a
   non-localizable technique reaching $N^{1+\varepsilon}$ for an HM-frontier problem.
   Meta-complexity is disqualified as the non-local ingredient (S2E relativizes, Chen-
   Hirahara-Ren STOC 2024; W2A core relativizes, Hirahara 2018), so the non-
   relativizing burden falls on the Williams spine (a direction, not a technique) and
   J4 (no combining theorem) is the composition obstacle. The single most-attackable J3
   sub-question: extend CHOPRS Lemma 41's multilinear-extension localization collapse
   from $\mathsf{AC}^0[p]$ to threshold-oracle gates (success closes the no-go for
   $\mathsf{TC}^0$; failure names a gap where a non-local $\mathsf{TC}^0$ technique
   could live), with the honest demotion that the gate-type-agnostic GIIKKT cap already
   places the seam essentially at the open $\mathsf{TC}^0$ lower-bound problem.
...
- All experiment modules run to completion (re-verified 2026-06-03): the Phase-0
  ...
  `circuit_complexity.e_fused_max_mm_escape`, `circuit_complexity.e_fused_max_mm_endgame`,
  and `circuit_complexity.e_j1_subblock_j3_locality` (the
...
| 2026-06-04 | TC0 spine: finish J1 + pivot to J3 (multi-agent: 3 surveys, 2 attack prongs, adversary per prong, verifier; surveys-first, no StructuredOutput failures) | TWO coordinates, no progress on the prize. J1 (finding 33): the sub-block non-linear per-round information lemma is re-judged OPEN, not partial. The builder's marginal-info telescope was a MEASUREMENT ARTIFACT (a $\Theta(d^2)$ signal model, not the gap-1-faithful $+(2d-1)$ increment; under the faithful increment the per-round info collapses to the estimator null floor), and the test family was not gap-1; route (a) (Simchowitz/Kacham-Woodruff) is linear-only and dead for spectral reads. Named missing object: per-round conditional location-info uniformity under adaptive sub-block choice; BGLWWZ-MIC-via-streaming the most-promising unbuilt route (two unbridged steps). J3 (finding 34): the BINDING joint is ATTACKED and mapped as an OPEN NO-GO. The locality barrier (CHOPRS JACM 2022 Def 11/Thm 2; GIIKKT ICALP 2019) is precise and broadly covering (polynomial method, approximate degree, random restrictions, reductions, approximation method via Pich 2024), downstream of natural proofs; evasion criterion named (non-localizable, $N^{1+\varepsilon}$, HM-frontier); meta-complexity disqualified as the non-local ingredient (S2E relativizes, CHR STOC 2024); the Williams spine is the only plausible source (a direction); J4 unchanged. Most-attackable sub-question: extend CHOPRS Lemma 41 from $\mathsf{AC}^0[p]$ to threshold-oracle gates (honestly demoted: the gate-type-agnostic GIIKKT cap makes this essentially the open $\mathsf{TC}^0$ problem). Corrections: GIIKKT exponent $0.49/d$ (the builder's $0.49/(d-1)$ contradicted the repo's e_leading_path_descent.py); the builder's TC0 win-win demoted. New experiment `circuit_complexity/e_j1_subblock_j3_locality.py` (exit 0, smoke 5/5; one scratch prong consolidated); LEARNINGS findings 33, 34; note subsection 4g; atlas J3 update. No progress on the prize. |
