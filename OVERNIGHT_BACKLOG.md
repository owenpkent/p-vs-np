...
- [x] **P3b.** (recurring) One new candidate per visit until 6 coordinates are logged.
...
- [x] **P4a.** Audit which `sorry`/placeholder predicates in `lean/PvsNP*.lean` can map to real Mathlib definitions; record findings in `lean/README.md`.
- [x] **P4b.** Replace one placeholder predicate with a real statement (or document why Mathlib cannot yet support it).
...
- 2026-06-02: P3b done. Added three more strand-3 coordinates (six total): pi_1 class of the solution nerve and Bockstein image beta(x) are non-algebrizing but OPEN (no cheap count); the discrete-Morse persistent mod-2 barcode is DEAD on strand 3 (it outputs ranks/Betti, which algebrize). Sharpest coordinate: even a cheap, torsion-AWARE computation algebrizes if it outputs ranks, so the decisive object is the FORCING bridge (a cheap count forcing a torsion class nonzero via a Bockstein sequence), not another invariant on one side. Driver runs to exit 0. P3 complete.
- 2026-06-02: P4a done. Audited the Lean skeleton against Mathlib and the toolchain (Lean 4.13.0 present). Findings recorded in lean/README.md: the skeleton is Mathlib-free (lakefile requires Mathlib but no module imports it); Basic.lean compiles cleanly with bare lean (0 sorry); SAT.lean has a REAL compile error (not a sorry) -- ([]:Clause).eval resolves to nonexistent List.eval because Clause is a reducible abbrev (line 65), fix is to call Clause.eval explicitly (now the P4b target); the #TM/#CL/#REL/#CKT targets are genuine gaps (Mathlib has Turing machines but no cost/poly-time class); the external LeanMillenniumPrizeProblems repo is the closest statement surface. Near-term win: a standalone Mathlib-free green build after the SAT fix.
- 2026-06-02: P4b done. Fixed two real Lean compile bugs and verified the whole skeleton compiles against core Lean (no Mathlib): (1) SAT.lean ([]:Clause).eval resolved to nonexistent List.eval because Clause is a reducible abbrev -> call Clause.eval explicitly; (2) Basic.lean RunsInPolyTime was typed BitString->Bool only, so a reduction's f:BitString->BitString did not typecheck -> made it polymorphic in the codomain (still the True placeholder, #TM-1). All five modules (Basic, SAT, CookLevin, Relativization, CircuitLowerBounds) now typecheck with bare lean; the documented sorry targets (#CL/#REL/#CKT) remain as open math. Updated lean/README.md status + audit + conclusion (standalone build is green) and the TODO Lean section.

## ALL BACKLOG TASKS COMPLETE (2026-06-02)

P1 (reading-notes synthesis: a-d), P2 (five experiments: a-e), P3 (strand-3 ledger: a-b), and P4 (Lean: a-b) are all done. The overnight orchestrated loop stops here. Summary: 13 tasks, each committed separately; LEARNINGS findings 11-19 added; six research directions / docs deepened; five new runnable experiments plus the strand-3 ledger driver, all passing self-checks; one propagated factual error corrected (MAJORITY approximate degree); two real Lean bugs fixed and the skeleton now compiles against core Lean.
