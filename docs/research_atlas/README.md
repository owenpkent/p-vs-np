...
open. The descent's binding joint (J3) is now ATTACKED-AND-MAPPED (LEARNINGS finding 34, 2026-06-04). The locality barrier (Chen-Hirahara-Oliveira-Pich-Rajgopal-Santhanam, JACM 2022, arXiv:1911.08297; Golovnev-Ilango-Impagliazzo-Kabanets-Kolokolova-Tal, ICALP 2019) is stated exactly: a lower-bound proof is LOCAL when it extends to the oracle-circuit model $[q,l,a]$-$\mathcal{C}$ (CHOPRS Def 11) for any small-fan-in oracle, hardness magnification unconditionally emits such oracle circuits (B1$^O$) and the known techniques extend to them (B3$^O$), so technique and target meet inside one class and cannot separate. The weak target HM needs is slightly superlinear $N^{1+\varepsilon}$, and local methods provably cannot reach it; coverage now includes the polynomial method, approximate degree, random restrictions, reductions, and Razborov's approximation method (Pich, Computational Complexity 2024). The barrier is downstream of, and distinct from, natural proofs (HM clears naturalness, CHOPRS Thm 1 (a)$\iff$(d)), confirming the escape flips from non-largeness to non-constructivity exactly at J3. Verdict: an OPEN NO-GO with a precise evasion criterion (a non-localizable technique reaching $N^{1+\varepsilon}$ for an HM-frontier problem). Meta-complexity is disqualified as the non-local ingredient (the S2E near-maximum engine, Chen-Hirahara-Ren STOC 2024, relativizes and avoids HM; the W2A core relativizes, Hirahara 2018), so the non-relativizing Williams spine carries the whole burden as a direction not a technique, and J4 (no combining theorem) remains the composition obstacle. The single most-attackable sub-question: whether the AC0[p] localization collapse (CHOPRS Lemma 41, a polynomial-method degree argument) extends to threshold oracle gates and hence to $\mathsf{TC}^0$.

Pushing on to $\mathsf{NC}^1$ / $\mathsf{P/poly}$ and to $\mathsf{NP}$ remains the
