...
## 4g. The sub-block non-linear per-round lemma is OPEN (the marginal-info telescope was an artifact), and J3 is attacked-and-mapped as an open no-go (findings 33-34)

Added 2026-06-04 (a pass running both dossier routes for the sub-block lemma with measured numbers, plus a first-principles map of J3 with adversary and verifier audits; the survey read Simchowitz-El Alaoui-Recht STOC 2018 / Kacham-Woodruff NeurIPS 2023 / BGLWWZ arXiv:2403.20283 / Racz-Schiffer ALEA 2020 / Mardia-Verchand-Wein arXiv:2402.05451 for Track 1, and CHOPRS JACM 2022 / GIIKKT ICALP 2019 / Pich Comp. Complexity 2024 / Chen-Hirahara-Ren STOC 2024 for Track 2). Subsection 4f left exactly one residual deterministic object, the sub-block non-linear per-round information lemma. This subsection reports it OPEN, not partial, and maps J3. Consolidated model: [`../../experiments/circuit_complexity/e_j1_subblock_j3_locality.py`](../../experiments/circuit_complexity/e_j1_subblock_j3_locality.py) (exit 0; self-check OK; smoke 5/5).

TRACK 1 (the lemma, OPEN). Route (a) is dead for spectral reads: the Simchowitz orthogonalize-WLOG deflation (Obs 3.1 / Sec 3.4) and Gaussian conditional-likelihood closure (Lemma 3.4) are linear-only, measured cross-block non-linear deflation residual $\approx 0.997$ vs linear $\approx 10^{-15}$. The adversary added a correction: degree-2 entry-symmetric MOMENTS tile EXACTLY (residual $0$), so a deflation DOES exist for the moment family and the no-port claim is sound only for non-additive spectral reads. Route (b)'s would-be closing telescope FAILS: its load-bearing per-round law $I\sim C\,b^2/n^2$ with $C\approx 4.5$ was a measurement artifact of an inconsistent signal model (a $\Theta(d^2)$ cell-upgrade, not the gap-1-faithful $+(2d-1)$ increment); under the faithful increment the per-round info collapses to the binned-MI estimator null floor (I/f at $n=128,d=64$ faithful $2.33/0.79/0.19/0.029/0.009$ vs null floor $1.77/0.50/0.089/0.026/0.007$ at $b=2,4,8,16,32$). The test family was also not gap-1 (planted $d=64$ over a bulk maxing at $24$, a $4\times$ gap a single read detects), and the Fano telescope bounds localization not the 1-bit detection task (proves too little). What survives as EVIDENCE not proof: per-round SNR $\Theta(1/(b\sqrt d))$ (proper sub-blocks sub-SNR at gap 1) and greedy-descent success decay $0.62/0.35/0.20/0.05$ at $n=64/128/256/512$. The named missing object is the per-round CONDITIONAL location-info uniformity $I(\text{answer}_k;L\mid\text{answer}_{<k})=O(b_k^2/n^2)$ under adaptive sub-block choice; the more promising route is the BGLWWZ MIC measure (Lemma 1.1, $\mathrm{MIC}\le 2ksn$, a SPACE bound agnostic to query linearity) with two unbridged steps: a streaming-simulation lemma and a gap-1-to-needle/MostlyEq reduction (BGLWWZ does NOT route through UDISJ).

TRACK 2 (J3, OPEN NO-GO). The locality barrier (CHOPRS Def 11; Thm 2 the coupled B1$^O$/B3$^O$ forbiddance) is real, broadly covering (polynomial method, approximate degree, random restrictions, reductions, and now the approximation method via Pich 2024), and downstream of natural proofs (HM clears naturalness, CHOPRS Thm 1 (a)$\iff$(d)). The weak target is $N^{1+\varepsilon}$; the evasion criterion is non-localizability (the argument breaks under $o(N)$ arbitrary fan-in-$N^\varepsilon$ oracle gates) at an HM-frontier problem. Meta-complexity is disqualified as the non-local ingredient: S2E (Chen-Hirahara-Ren STOC 2024, arXiv:2309.12912) RELATIVIZES and avoids HM, and the W2A core relativizes (Hirahara 2018), so the Williams spine carries the entire non-relativizing burden (a direction, not a technique). GIIKKT cap exponent corrected to $\exp(\Omega(N^{0.49/d}))$ ($0.245/0.1633/0.1225/0.0817$ at $d=2/3/4/6$). The TC0 seam is DEMOTED: Lemma 41's localization collapse is proved at $\mathsf{AC}^0[p]$ via the multilinear-extension degree argument and does not obviously port to high-degree THRESHOLD oracle gates, but the GIIKKT computational cap ($\mathrm{MAJ},\mathsf{NC}^1\in(\mathsf{AC}^0)^{\mathrm{MCSP}}$) is gate-type-agnostic and already places MCSP-oracle devices above $\mathsf{AC}^0[p]$ without that argument, so the seam is the open $\mathsf{TC}^0$ lower-bound problem relabeled. The most-attackable probe remains: try to extend Lemma 41 to threshold oracle gates (success $\Rightarrow$ no-go extends to $\mathsf{TC}^0$; failure $\Rightarrow$ a named gap).

NET. One deterministic residual object survives, the sub-block non-linear per-round information lemma, now confirmed OPEN with a named missing object; J3 is mapped as a precise open no-go with one attackable sub-question; J4's non-relativizing burden falls entirely on the spine. Findings 33-34. No speedup claimed; $\mathsf{NEXP}\not\subseteq$ poly-size THR-of-THR remains open as of June 2026.
